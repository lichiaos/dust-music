  ...
  <div class="player" v-show="playlist">
    ...
    >
      <div class="normal-player" v-show="fullScreen">
        <div class="background" :class="cdClass">
        ...
        <div class="bottom">
          <div class="progress-wrapper">
            <span class="time time-l">{{ _fmtSongTime(currentTime) }}</span>
            <div class="progress-bar-wrapper">
              <progress-bar :percent="percent" @progressChange="progressChange"></progress-bar>
            </div>
            <span class="time time-r">{{ _fmtSongTime(currentSong.duration) }}</span>
          </div>
          <div class="operators">
...
export default {
  ...
  methods: {
    ...
    resetIndex(list) {
      const index = list.findIndex(k => k.id === this.currentSong.id)
    ...
    _fmtSongTime(val) {
      if (String(val).length === 6) val /= 1000
      val = val | 0
      const m = (val / 60) | 0
      const s = this._pad(val % 60)
      return `${m}:${s}`
    }
  },
  watch: {
